...
const CheckoutPage: React.FC = () => {
  ...
  useEffect(() => {
    const getCartItems = () => {
      // Try to get items from location state first (passed from navigation)
  ...
  useEffect(() => {
    if (orderItems.length === 0 && !error) {
      setTimeout(() => {
        navigate('/cart');
      }, 2000);
    }
  }, [orderItems, navigate, error]);
  ...
  const handlePlaceOrder = async () => {
    try {
      ...
      // Create orders for each item (each product has its own order in our system)
      const newOrders = [];

      for (const item of orderItems) {
        try {
          console.log(`📦 Creating order for product: ${item.name} (ID: ${item.id})`);
          ...
          if (!validation.isValid) {
            throw new Error(`Validation failed for ${item.name}: ${validation.errors.join(', ')}`);
          }
          ...
          newOrders.push(orderResponse);

        } catch (itemError) {
          console.error(`❌ Failed to create order for ${item.name}:`, itemError);
          throw new Error(`Failed to create order for ${item.name}: ${itemError instanceof Error ? itemError.message : 'Unknown error'}`);
        }
      }
      ...
      setTimeout(() => {
        navigate('/confirmation', { state: { orders: newOrders } });
      }, 2000);
              ...
              <div className="p-6">
                {orderItems.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">No Items in Cart</h3>
                    <p className="text-gray-600 mb-4">Your cart is empty. Redirecting you to the cart page...</p>
                    <button
