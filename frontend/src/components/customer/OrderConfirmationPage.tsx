...
const OrderConfirmationPage: React.FC = () => {
  ...
  // Generate order number and customer info from location state or defaults
  const orderNumber = orderData?.orders?.[0]?.orderNumber || `#ANZ-${new Date().getFullYear()}-${Math.floor(Math.random() * 1000000).toString().padStart(6, '0')}`;
  const customerName = orderData?.customerInfo?.name || 'Customer';
  const customerEmail = orderData?.customerInfo?.email || 'customer@example.com';

  // Extract order items from order data
  const orderItems: OrderItem[] = orderData?.orders?.map(order => ({
    id: order.id?.toString() || Math.random().toString(36).substr(2, 9),
    name: order.product?.name || 'Product',
    quantity: order.quantity || 1,
    price: order.product?.price || 0,
    image: order.product?.image || 'https://storage.googleapis.com/uxpilot-auth.appspot.com/83eec471e4-bf3e193df121a42fe3da.png',
  ...
  useEffect(() => {
    // Load order data from location state
    const loadOrderData = () => {
      try {
        if (location.state?.orders) {
          setOrderData({ orders: location.state.orders });
        } else {
