...
class ProductController {
    ...
    async createProduct(req, res) {
        try {
            ...
            const { name, description, shortDescription, price, salePrice, sku, stockQuantity, lowStockAlert, commission, category, subcategory, weight, shippingClass, dimensions, videoUrl, metaTitle, metaDescription, status } = req.body;
            ...
            try {
                const sellerProfile = await database_1.default.$queryRaw `SELECT commission_rate FROM tbl_seller_profiles WHERE user_id = ${userId}`;
                if (sellerProfile && sellerProfile.length > 0 && sellerProfile[0].commission_rate) {
                    defaultCommission = sellerProfile[0].commission_rate;
                }
            }
            catch (error) {
            ...
            // Create the product
            const newProduct = await database_1.default.tbl_products.create({
                data: {
            ...
            if (req.files && Array.isArray(req.files)) {
                for (const file of req.files) {
                    await database_1.default.pro_photo.create({
                        data: {
    ...
    async getVendorProducts(req, res) {
        try {
            ...
            try {
                sellerProfile = await database_1.default.$queryRaw `SELECT * FROM tbl_seller_profiles WHERE user_id = ${userId}`;
            }
            catch (error) {
            ...
            const formattedProducts = products.map((product) => ({
                ...
                images: product.pro_photo.map((photo) => ({
                    id: photo.p_ID,
                    path: photo.p_path,
                })),
                sellerInfo: {
                    id: userId,
                    businessName: sellerProfile && sellerProfile.length > 0 ? sellerProfile[0].business_name : null,
                    verificationStatus: sellerProfile && sellerProfile.length > 0 ? sellerProfile[0].verification_status : 'pending'
                }
            }));
            res.json({
    ...
    async getProductById(req, res) {
        try {
            const { id } = req.params;
            const userId = req.user?.userId;
            const product = await database_1.default.tbl_products.findFirst({
                where: {
                    pro_ID: parseInt(id),
                    seller_nm: userId,
                },
    ...
    async updateProduct(req, res) {
        try {
            ...
            const existingProduct = await database_1.default.tbl_products.findFirst({
                where: {
                    pro_ID: parseInt(id),
                    seller_nm: userId,
                },
            ...
            const updatedProduct = await database_1.default.tbl_products.update({
                where: {
                    pro_ID: parseInt(id),
                },
                data: {
    ...
    async deleteProduct(req, res) {
        try {
            ...
            await database_1.default.tbl_products.delete({
                where: {
                    pro_ID: parseInt(id),
                },
            });
    ...
    async getProductsForCustomers(req, res) {
        try {
            ...
            const productsWithSellers = await Promise.all(products.map(async (product) => {
                ...
                let sellerProfile = null;
                try {
                    sellerProfile = await database_1.default.$queryRaw `SELECT business_name, verification_status, rating FROM tbl_seller_profiles WHERE user_id = ${product.seller_nm}`;
                }
                catch (error) {
                ...
                return {
                    ...
                    seller: seller ? {
                        ...
                        businessName: sellerProfile && sellerProfile.length > 0 ? sellerProfile[0].business_name : null,
                        verificationStatus: sellerProfile && sellerProfile.length > 0 ? sellerProfile[0].verification_status : 'pending',
                        rating: sellerProfile && sellerProfile.length > 0 ? sellerProfile[0].rating : 0
                    } : null
                };
            }));
            const total = await database_1.default.tbl_products.count({
                where: whereClause
            });
            res.status(200).json({
                success: true,
    ...
    async getProductDetailsForCustomers(req, res) {
        try {
            const { id } = req.params;
            const product = await database_1.default.tbl_products.findFirst({
                where: {
            ...
            let sellerProfile = null;
            try {
                sellerProfile = await database_1.default.$queryRaw `SELECT * FROM tbl_seller_profiles WHERE user_id = ${product.seller_nm}`;
            }
            catch (error) {
                // Profile table doesn't exist
            }
            const productDetails = {
                ...
                seller: seller ? {
                    ...
                    businessName: sellerProfile && sellerProfile.length > 0 ? sellerProfile[0].business_name : null,
                    businessDescription: sellerProfile && sellerProfile.length > 0 ? sellerProfile[0].business_description : null,
                    businessEmail: sellerProfile && sellerProfile.length > 0 ? sellerProfile[0].business_email : null,
                    businessPhone: sellerProfile && sellerProfile.length > 0 ? sellerProfile[0].business_phone : null,
                    verificationStatus: sellerProfile && sellerProfile.length > 0 ? sellerProfile[0].verification_status : 'pending',
                    rating: sellerProfile && sellerProfile.length > 0 ? sellerProfile[0].rating : 0
                } : null
            };
