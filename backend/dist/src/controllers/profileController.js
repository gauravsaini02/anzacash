...
class ProfileController {
        ...
        try {
            ...
            res.status(200).json({
                ...
                data: {
                    ...
                    profile: sellerProfile ? sellerProfile[0] : null,
                    finances: {
    ...
    async updateSellerProfile(req, res) {
        try {
            ...
            try {
                const existingProfile = await database_1.default.$queryRaw `SELECT * FROM tbl_seller_profiles WHERE user_id = ${userId}`;
                if (existingProfile && existingProfile.length > 0) {
                    ...
                    if (updateFields.length > 0) {
                        values.push(userId);
                        await database_1.default.$queryRaw `UPDATE tbl_seller_profiles SET ${updateFields.join(', ')} WHERE user_id = ${values[values.length - 1]}`;
                    }
                }
                else {
                    // Create new profile
    ...
    async getCustomerProfile(req, res) {
        try {
            ...
            try {
                customerProfile = await database_1.default.$queryRaw `SELECT * FROM tbl_customer_profiles WHERE user_id = ${userId}`;
            }
            catch (error) {
                customerProfile = null;
            }
            // Get customer stats
            const ordersCount = await database_1.default.tbl_sh_orders.count({
            ...
            res.status(200).json({
                ...
                data: {
                    ...
                    stats: {
                        ...
                        loyaltyPoints: customerProfile?.[0]?.loyalty_points || 0
                    }
                }
            });
        }
    ...
    async updateCustomerProfile(req, res) {
        try {
            ...
            try {
                const existingProfile = await database_1.default.$queryRaw `SELECT * FROM tbl_customer_profiles WHERE user_id = ${userId}`;
                if (existingProfile && existingProfile.length > 0) {
                    // Update existing profile
                    const updateFields = [];
                    const values = [];
                    ...
                    if (updateFields.length > 0) {
                        values.push(userId);
                        await database_1.default.$queryRaw `UPDATE tbl_customer_profiles SET ${updateFields.join(', ')} WHERE user_id = ${values[values.length - 1]}`;
                    }
                }
                else {
                    // Create new profile
    ...
    async getPublicSellerProfile(req, res) {
        try {
            ...
            let sellerProfile = null;
            try {
                sellerProfile = await database_1.default.$queryRaw `SELECT * FROM tbl_seller_profiles WHERE user_id = ${parseInt(sellerId)}`;
            }
            catch (error) {
                sellerProfile = null;
            }
            ...
            const productsWithImages = await Promise.all(products.map(async (product) => {
                const photo = await database_1.default.pro_photo.findFirst({
