...
class VendorController {
    async getVendorProfile(req, res) {
        try {
            ...
            const productsCount = await database_1.default.tbl_products.count({
    ...
    async getVendorStats(req, res) {
        try {
            ...
            const activeProducts = products.filter((p) => p.pro_status === 'Approved').length;
            const pendingProducts = products.filter((p) => p.pro_status === 'Not Approved').length;
            const totalOrders = orders.length;
            const completedOrders = orders.filter((o) => o.oda_status === 'Completed').length;
    ...
    async updateVendorProfile(req, res) {
        try {
            ...
            // Update user profile
            const updatedUser = await database_1.default.nasso_users.update({
                where: { usr_Id: userId },
                data: {
