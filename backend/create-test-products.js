...
function generateProducts(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const templateIndex = Math.floor(i / categories.length) % productTemplates[category].length;
    const template = productTemplates[category][templateIndex];
...
console.log('Saved 100 products to test-products.json');

// Now create individual SQL files for each product for easier manual insertion
console.log('Creating SQL insert statements...');

let sqlStatements = `
-- Insert test products for seller ID 103669
-- Make sure to run these SQL statements in your database
`;

products.forEach((product, index) => {
  const quotedName = product.name.replace(/'/g, "''");
  const quotedDescription = product.description.replace(/'/g, "''");
  const quotedCategory = product.category.replace(/'/g, "''");
  const quotedLocation = product.location.replace(/'/g, "''");

  sqlStatements += `
INSERT INTO tbl_products (name, description, price, category, commission, stock, location, country, seller_id, created_at)
VALUES ('${quotedName}', '${quotedDescription}', ${product.price}, '${quotedCategory}', ${product.commission}, ${product.stock}, '${quotedLocation}', '${product.country}', 103669, '${product.date}');
`;
});

fs.writeFileSync('insert-test-products.sql', sqlStatements);
console.log('Saved SQL statements to insert-test-products.sql');
console.log('You can now run this SQL file in your database to add the products.');